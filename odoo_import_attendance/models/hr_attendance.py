...
class HRAttendance(models.Model):
    ...
    def _check_validity(self):
        ...
        for attendance in self:
            ...
            #         ('id', '!=', attendance.id),
            #     ], order='check_in desc', limit=1)
            #     if no_check_out_attendances and not self.env.context.get('esc'):
            #         raise exceptions.ValidationError(
            ...
            if last_attendance_before_check_out and last_attendance_before_check_in and last_attendance_before_check_in != last_attendance_before_check_out:
                raise exceptions.ValidationError(
                    _("Cannot create new attendance record for %(empl_name)s, the employee was already checked in on %(datetime)s") % {
                        'empl_name': attendance.employee_id.name,
