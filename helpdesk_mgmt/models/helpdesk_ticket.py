...
class HelpdeskTicket(models.Model):
    ...
    def write(self, vals):
        if vals:
            users = self.team_id.user_ids.ids + self.team_id.user_id.ids + self.user_id.ids
            flag = self.env.user.has_group('helpdesk_mgmt.group_helpdesk_user') and  not self.env.user.has_group('helpdesk_mgmt.group_helpdesk_manager')
            if self.env.user.id not in users and flag :
                raise exceptions.ValidationError("You Are Allowed to edit in you tickets and tickets of you team only")
